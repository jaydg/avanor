...
bool XGuiItem_Text::SetWidth(int width)
{
	...
	while(true)
	{
		...
		if(x_strlen(str) == 0) break;

		int last_space = -1, size = 0, len = 0;
		while(len <= width)
		{
			if(str[size] == 0x1F) { size += 2; continue; }
			if(str[size] == 0) { is_last_line = true; last_space = size; break; }
			if(str[size] == '\n') { is_line_break = true; last_space = size; break; }
			if(str[size] == ' ') last_space = size;
			size++; len++;
		}
		...
		char * p = tmp; while(*p != 0) if(*p++ == 0x1F) color = *p++;

		if(!is_last_line && !is_line_break) WideBuffer(tmp, width);
...
void XGuiList::LineDown(int count)
{
	while(count > 0)
	{
		if(top_line + list_height >= lines_count) return;
