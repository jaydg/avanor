...
int XEffect::GetRange(EFFECT effect, int power)
{
	switch (effect)
	{
		case E_MAGIC_ARROW: return power / 4 + 2;
		case E_FIRE_BOLT: return power / 6 + 2;
		case E_ICE_BOLT: return power / 6 + 2;
		case E_LIGHTNING_BOLT: return power / 7 + 2;
		case E_ACID_BOLT: return power / 8 + 2;
		case E_TELEPORT: return 1;
		default: return 0;
	}
}
	...
	if (caster->onHeal(d.S))
	{
		if (caster->isVisible())
		{
			...
			if (caster->GetMaxHP() == caster->_HP)
				...
				if (d.S < 8) msgwin.Add("slightly healed.");
				else if (d.S < 15) msgwin.Add("somewhat healed.");
				else if (d.S < 100) msgwin.Add("healed.");
				else msgwin.Add("greatly healed.");
		}
		return 1;
	} else
		return 0;
