        ...
        case START:
            if (cr->isHero()) {
                ...
                if (cr->GetTarget(TR_ATTACK_DIRECTION, &pt)) {
                    ...
                    if (XTileType::DiggableInto(cr->l->map->GetXY(tgt_x, tgt_y)) != XTileType::NONE) {
                        rock_resist = 1000;
            ...
            if (rock_resist < 0) {
                if (cr->isHero()) {
                    msgwin.Add(fmt::format(
                        ...
                        cr->GetNameEx(CRN_T1),
                        cr->GetVerb("smash")));
                    cr->l->map->SetXY(tgt_x, tgt_y, XTileType::DiggableInto(cr->l->map->GetXY(tgt_x, tgt_y)));
...
{
    if (item->kind & ItemKind::FOOD && item->it == ItemType::HERB) {
        return 1;
    } else {
        return 0;
    }
