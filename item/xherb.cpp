...
int XHerbBush::CountNeighbours(int x, int y)
{
	...
	for(int i = x - 1; i <= x + 1; i++)
		for(int j = y - 1; j <= y + 1; j++)
		{
			...
			if(obj == 0 || !obj->isValid() || obj->im != IM_HERB) continue;
			N++;
		}
	return N;
}
...
int XHerbBush::Run()
{
	...
	if(N == 3 && herb_strength < 3) herb_strength++;

	for(int i = x - 1; i <= x + 1; i++)
		for(int j = y - 1; j <= y + 1; j++)
		{
			XMapObject * obj = l->map->GetSpecial(i, j);
			if(obj != 0 || l->map->GetXY(i, j) != M_GREENGRAS) continue;
			if(CountNeighbours(i, j) != 3) continue;
			new XHerbBush(i, j, l);
		}
