...
int vGetch()
{
	...
	if (ch != 27) return ch;
   	...
   	if (ch == ERR) return KEY_ESC;
   	if (ch != '[') { ungetch(ch); return KEY_ESC; }
   	...
   	switch (getch())
   	{
   		...
   		case 'G': return KEY_CENTER;
   		case '1': return getch() == '~' ? KEY_HOME : KEY_UNKNOWN;
   		case '4': return getch() == '~' ? KEY_END : KEY_UNKNOWN;
   		case '5': return getch() == '~' ? KEY_PGUP : KEY_UNKNOWN;
   		case '6': return getch() == '~' ? KEY_PGDOWN : KEY_UNKNOWN;
   	}
   	return KEY_UNKNOWN;
#else
	int ch = getch();
	if (ch == 0 || ch == 224) ch = KEY_EXTENDED_CODE | getch();
	return ch;
#endif
}
...
{
	if(cursor_pos_y >= size_y) cursor_pos_x = cursor_pos_y = 0;
	...
	while(true)
	{
		...
		switch(ch)
		{
			...
			case '\n' :
				cursor_pos_x = 0;
				if(++cursor_pos_y >= size_y) cursor_pos_y = 0;
				break;
			default :
			{
