...
public:
	...
	protected:
		...
		static XNode * CreateNode() { return new XNode(); }
		static void DestroyNode(XNode * pNode)	{ delete pNode;	}
	...
	public:
		...
		void operator--() { assert(position); position = position->pPrev; }
		void operator--(int) { operator--(); }
...
//			assert(position->o && position->o->isValid()); 
	...
	void StoreList(XFile * f)
	{
		...
		for(iterator it = begin(); it != end(); ++it) XObject::StorePointer(f, it);
	}
